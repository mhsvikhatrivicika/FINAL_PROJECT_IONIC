...
@Component({
  selector: 'app-scan-dosen',
  templateUrl: './scan-dosen.page.html',
  styleUrls: ['./scan-dosen.page.scss'],
})
export class ScanDosenPage implements OnInit, OnDestroy {
  absen: any;
  absen_lab: any;
  absen_name: any;
  ...
  async startScanner() {
    ...
    if (result.hasContent) {
      ...
      if (token) {
        try {
          this.api.getBeforeAbsenDosen(token, result.content).subscribe(
            async (response: any) => {
                ...
                buttons: [
                  {
                    ...
                    handler: async () => {
                      ...
                      this.router.navigate(['tabs-dosen']);
                  ...
                  },
                  {
                    ...
                    handler: async () => {
                      ...
                      try {
                        this.api.absenDosen(this.absensi, token).subscribe(
                          async (data: any) => {
                            const toast = await this.toastController.create({
            ...
            async (err) => {
              ...
              this.router.navigate(['tabs-dosen/home-dosen']);
            }
          );
        } catch (error) {
        }
